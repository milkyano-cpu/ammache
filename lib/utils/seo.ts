...
import type { ProjectWithCategory } from "@/lib/types/project"

const SITE_URL = "https://ammache.com"
...
export function generateProductMetadata(product: ProjectWithCategory): Metadata {
...
export function generateProductJsonLd(product: ProjectWithCategory) {
  const image =
    product.images?.[0]?.startsWith("http")
      ? product.images[0]
      : product.images?.[0]
